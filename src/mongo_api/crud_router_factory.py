...
CreateSchema/UpdateSchema/actual_model are built dynamically as LOCAL
variables inside create_crud_router(). With postponed evaluation active,
FastAPI resolves string annotations using the route function's __globals__
(the module namespace) — it cannot see these closure-local names, and
route registration fails with NameError the moment create_crud_router()
runs. Keep annotations as real (non-string) objects in this file.
"""
...
def create_crud_router(
    collection_name: str,
    model: Optional[Type[MongoBaseModel]] = None,  # optional
    *,
    ...
    # ------------------------------------------------------------------
    # 3. Repository — built ONCE per router (lazily, on first request),
    # not once per request. Avoids re-running create_index() on every
    # single API call, which is wasted latency (and risk of timeouts
    # on serverless) for no benefit, since indexes are idempotent.
    # ------------------------------------------------------------------
    _repo_cache: dict[str, BaseRepository] = {}

    def get_repo() -> BaseRepository:
        if "repo" not in _repo_cache:
            _repo_cache["repo"] = BaseRepository(
